...
@app.get("/createRecipe", response_model=models.RECIPES)
async def createRecipe():#(recipejson: str):
    db = Prisma()
    await db.connect()
    # Create a new recipe
    ...
    print(f'loaded recipe: {json.dumps(recipe, indent=2)}')
    imported_recipe = await import_recipes(recipe, output_id=True, debug=True, verbose=True)
    print(f'imported recipe: {imported_recipe.json(indent=2)}')
    await db.disconnect()
